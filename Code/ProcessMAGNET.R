...
regMAGNET2agCLIM50 <- read_csv("Mappings/regMAGNET2agCLIM50II.csv") %>%
  unique()

secMAGNET2agCLIM50 <- read_csv("Mappings/secMAGNET2agCLIM50II.csv") %>%
  dplyr::select(-Note) %>%
  unique()
...
write_csv(MAGNET_tot, paste("Cache/agCLIM50II_MAGNET_", Sys.Date(), ".csv", sep=""))
